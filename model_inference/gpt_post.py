...
import tot
import re
from run_lexicon import create_index, METRICS
import ir_datasets
...
def create_title_index(
    dataset, dest_folder, index, gather_wikidata_aliases, wikidata_cache
):
    ...
    with open(os.path.join(docs_folder, "docs.jsonl"), "w") as writer:
        for raw_doc in dataset.docs_iter():
            doc = {"id": raw_doc.doc_id, "contents": "\n".join(aliases[raw_doc.doc_id])}
...
def resolve(
    title, matched_title, title_to_doc_id, aliases, scorer, assert_perfect_score=False
):
    gen = []
...
class WikiCache:
    ...
    def get(self, wid):
        ...
        utils.write_json(
            {"entity": ent, "retrieved_on": datetime.now().isoformat()}, cache_path
        )
        return ent
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "gpt_post", description="post process outputfrom GPT, and compute run"
    )
    ...
    parser.add_argument(
        "--ref_run", default=None, help="if provided, this run is used to break ties"
    )

    parser.add_argument(
        "--metrics",
        required=False,
        default="recall_1,recall_10,recall_20,ndcg_cut_20,recip_rank",
        help="csv - metrics to evaluate",
    )
    parser.add_argument(
        "--docs_path",
        default="./anserini_title_docs",
        help="path to store (temp) documents for indexing",
    )
    parser.add_argument(
        "--index_path",
        default="./anserini_title_indices",
        help="path to store (all) indices",
    )
    parser.add_argument(
    ...
    for title in tqdm(titles):
        if title in title_to_doc_id:
            gen_title_to_doc_ids[title] = resolve(
                title=title,
                matched_title=title,
            ...
            if matched is None:
                unmatched.add(title)
                unmatched_props[title] = {"choices": choices}
                continue
            ...
            matched_nobr = process.extractOne(
                remove_non_alpha(remove_braces(title)), choices_nobr
            )
    ...
    for query in queries:
        ...
        for rank, title in zip(ranks, query["gpt_queries"]):
            ...
            # otherwise re-order based on reference run
                ...
                for srank, (doc_id, _) in enumerate(
                    sorted(ref_scores.items(), key=lambda _: _[1])
                ):
                    rank += step
                    run[qid][doc_id] = rank
