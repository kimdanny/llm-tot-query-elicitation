...
class TrecToTDocs(BaseDocs):
    ...
    def docs_store(self, field='doc_id'):
        return PickleLz4FullStore(
            path=f'{ir_datasets.util.home_path()}/trec-tot24/docs.pklz4', # tot 24 version
            # path=f'{ir_datasets.util.home_path() / NAME}/docs.pklz4', # tot 23 version
            init_iter_fn=self._docs_iter,
            data_cls=self.docs_cls(),
            lookup_field=field,
            index_fields=[field],
            count_hint=3185450 # tot 24 version
            # count_hint=231852 # tot 23 version
        )
